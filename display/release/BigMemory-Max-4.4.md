...
`6.0 Cautions and Warnings`
  ...
  `<install-dir>/common/runtime/bundles/tes/eclipse/plugins/ directory, but` 
  `what is important is that only the latest version (e.g. 4.3.10.16_10) of the`
  `org.terracotta.toolkit-runtime-ee library and latest version (e.g. 2.10.10.16_10)`
  `of net.sf.ehcache.ee would be loaded and used by terracotta clients (such as`
  `Integration Server). The older versions will not be loaded into any run-times.`
  `Please ignore the old JAR files after fix installation.`

  `Servers should be updated with new version before clients are updated. Older`
  `clients are recognized by newer servers in the 4.3 codebase, however newer`
  `updated clients will not be recognized by older servers.`
